...
class Qwen2VLGRPOVLLMTrainerModified(Trainer):
    ...
    def compute_loss(
        ...
        if data_type == 'image':
            input_copy[0]['content'][0]['image'] = os.getcwd() + "/dataset" + inputs[0]['path'][1:] 
        elif data_type == 'video':
            input_copy[0]['content'][0]['video'] = os.getcwd() + "/dataset" + inputs[0]['path'][1:] 
        ...
        if self.temporal:
            if video_inputs:
                ...
                shuffled_mm_data = [[self.accelerator.process_index, data_type, image_inputs if image_inputs else shuffled_video_inputs]]
